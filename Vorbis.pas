...
//    Version 1.1      Mike Looijmans - For CD Wave integration
unit Vorbis;

interface

uses windows, mmsystem, SysUtils;  //type definitions

const

   WAVE_FORMAT_OGG_VORBIS = $FF0F;
...
type
  ...
  POGGWaveFormat = ^TOGGWaveFormat;
  TOGGWaveFormat = packed record
...
var
  ...
  // DECODER SPECIFIC EXPORT FUNCTIONS
   ...
   vbDecRead : function(pvbDecInfo : PTVB_DEC_INFO;
		        pOutBuffer : PBYTE;
		        var pdwBytesOut : DWORD) : VB_ERR; cdecl;
   ...
   procedure InitVorbisDLL;
   procedure OGGCall(oggResult: VB_ERR);
   function VorbisDLLVersionStr : string;

implementation

const
     VorbisEncDLL : THANDLE = INVALID_HANDLE_VALUE;

procedure InitVorbisDLL;
var dll: THANDLE;
begin
   if (VorbisEncDLL = INVALID_HANDLE_VALUE) then
   begin
      dll := LoadLibrary('Vorbis.DLL');
      if (dll <> 0) then
      begin
         VorbisEncDLL := dll;
         @vbEncOpen   := GetProcAddress(dll,'vbEncOpen');
         @vbEncWrite  := GetProcAddress(dll,'vbEncWrite');
         @vbEncClose  := GetProcAddress(dll,'vbEncClose');
         @vbDecOpen   := GetProcAddress(dll,'vbDecOpen');
         @vbDecRead   := GetProcAddress(dll,'vbDecRead');
         @vbDecSeek   := GetProcAddress(dll,'vbDecSeek');
         @vbDecClose  := GetProcAddress(dll,'vbDecClose');
         @vbVersion   := GetProcAddress(dll,'vbVersion');
      end else
         RaiseLastWin32Error;
   end;
end;
...
procedure OGGCall(oggResult: VB_ERR);
begin
  if oggResult <> VB_ERR_OK then
    raise Exception.Createfmt('OGG Vorbis error: %d', [oggResult]);
end;
