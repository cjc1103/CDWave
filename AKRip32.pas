 ...
 * 03. January 2004:  (mike@cdwave.com) Made the unit load the dll dynamically
 *                    so it can be used 'optionally'.
 *
 **********************************************************************
 *
...
const
  ...
  akriplib = 'MyAKRIP.dll';

type
 TGetNumAdapters = function:Integer; cdecl;
 TGetCDList = function(var cd: CDLIST): Integer; cdecl;
 TGetAspiLibError= function: Integer; cdecl;
 TGetAspiLibAspiError = function: Byte; cdecl;
 TGetCDId = function(hCD: HCDROM; buf: PChar; maxBuf: Integer): DWord; cdecl;
 TGetDriveInfo = function(ha, tgt, lun: byte; var cdrec: CDREC): DWord; cdecl;
 TReadTOC = function(hCD: HCDROM; var MyToc: TOC; bMSF: LongBool): DWord; cdecl;
 TReadCDAudioLBA = function(hCD: HCDROM; TrackBuf: PTRACKBUF): DWord; cdecl;
 TQueryCDParms = function(hCD: HCDROM; which: Integer; var Num: DWord): Bool; cdecl;
 TModifyCDParms = function(hCD: HCDROM; which: Integer; val: DWord): Bool; cdecl;
 TGetCDHandle = function(var cd: GETCDHAND): HCDROM; cdecl;
 TCloseCDHandle = function(hCD: HCDROM): Bool; cdecl;
 TReadCDAudioLBAEx = function(hCD: HCDROM; TrackBuf, Overlap: PTRACKBUF): DWord; cdecl;
 TGetAKRipDllVersion = function: DWORD; cdecl;
...
type
 TGetCDDBDiskID = function(hCD: HCDROM; var pID: array of DWord; numEntries: Integer): DWord; cdecl;
 TfCDDBQuery = function(hCD: HCDROM; var lpq: TCDDBQUERY): DWORD; cdecl;
 TCDDBGetDiskInfo = function(var lpq: CDDBQUERYITEM; szCDDBEntry: PChar; maxLen: Integer): DWORD; cdecl;
 TCDDBSetOption = procedure(what: Integer; szVal: PChar; iVal: Integer); cdecl;
 TCDDBGetServerList = function(var lpSiteList: CDDBSITELIST): DWord; cdecl;

const
 GetNumAdapters: TGetNumAdapters = nil;
 GetCDList: TGetCDList = nil;
 GetAspiLibError: TGetAspiLibError = nil;
 GetAspiLibAspiError: TGetAspiLibAspiError = nil;
 GetCDId: TGetCDId = nil;
 GetDriveInfo: TGetDriveInfo = nil;
 ReadTOC: TReadTOC = nil;
 ReadCDAudioLBA: TReadCDAudioLBA = nil;
 QueryCDParms: TQueryCDParms = nil;
 ModifyCDParms: TModifyCDParms = nil;
 GetCDHandle: TGetCDHandle = nil;
 CloseCDHandle: TCloseCDHandle = nil;
 ReadCDAudioLBAEx: TReadCDAudioLBAEx = nil;
 GetAKRipDllVersion: TGetAKRipDllVersion = nil;
 GetCDDBDiskID: TGetCDDBDiskID = nil;
 CDDBQuery: TfCDDBQuery = nil;
 CDDBGetDiskInfo: TCDDBGetDiskInfo = nil;
 CDDBSetOption: TCDDBSetOption = nil;
 CDDBGetServerList: TCDDBGetServerList = nil;

var
 AKRipDLLLoaded: Boolean = False;

function BigToLittleEndian(a: TBigEndian): DWORD;
//function MSFtoLBA(a: TBigEndian): DWORD;
function NewTrackBuf(numFrames: Cardinal): PTRACKBUF;
procedure DisposeTrackBuf(buf: PTRACKBUF);

resourcestring
  ReadTOCError = 'Error reading TOC';
...
var ripdll: THANDLE;
begin
  ripdll := LoadLibrary(akriplib);
  if ripdll <> 0 then
  begin
    GetNumAdapters := GetProcAddress(ripdll, 'GetNumAdapters');
    GetCDList := GetProcAddress(ripdll, 'GetCDList');
    GetAspiLibError := GetProcAddress(ripdll, 'GetAspiLibError');
    GetCDId := GetProcAddress(ripdll, 'GetCDId');
    ReadTOC := GetProcAddress(ripdll, 'ReadTOC');
    ReadCDAudioLBA := GetProcAddress(ripdll, 'ReadCDAudioLBA');
    QueryCDParms := GetProcAddress(ripdll, 'QueryCDParms');
    ModifyCDParms := GetProcAddress(ripdll, 'ModifyCDParms');
    GetCDHandle := GetProcAddress(ripdll, 'GetCDHandle');
    CloseCDHandle := GetProcAddress(ripdll, 'CloseCDHandle');
    ReadCDAudioLBAEx := GetProcAddress(ripdll, 'ReadCDAudioLBAEx');
    GetAKRipDllVersion := GetProcAddress(ripdll, 'GetAKRipDllVersion');
    GetCDDBDiskID := GetProcAddress(ripdll, 'GetCDDBDiskID');
    CDDBQuery := GetProcAddress(ripdll, 'CDDBQuery');
    CDDBGetDiskInfo := GetProcAddress(ripdll, 'CDDBGetDiskInfo');
    CDDBSetOption := GetProcAddress(ripdll, 'CDDBSetOption');
    CDDBGetServerList := GetProcAddress(ripdll, 'CDDBGetServerList');
    AKRipDLLLoaded := True;
  end;
end;
...
{function MSFtoLBA(a: TBigEndian): DWORD;
begin
  Result := a[3] + 75 * a[2] + (60*75) * a[1];
end;}

//-- helper function for ripping - see below

function NewTrackBuf(numFrames: Cardinal): PTRACKBUF;
begin
  //-- Allocate memory for numFrames Frames plus TrackBuf-Data
  Result := PTRACKBUF(GlobalAlloc(GMEM_FIXED, (numFrames * 2352) + TRACKBUFEXTRA));
  if Result = nil then RaiseLastWin32Error;
  //-- initialize TrackBuf-Data
  Result^.startFrame := 0;
  Result^.numFrames := 0;
  Result^.maxLen := numFrames * 2352;
  Result^.len := 0;
  Result^.status := 0;
